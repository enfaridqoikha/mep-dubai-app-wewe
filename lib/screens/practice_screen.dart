...
class _PracticeScreenState extends State<PracticeScreen> {
  ...
  Future<void> _load() async {
    ...
    final qs = await QuestionService.loadPractice(lang, count: 20);
    setState(() {
      _lang = lang;
      _questions = qs;
      _index = 0;
  ...
  void _finish(){
    showDialog(
      ...
      builder: (ctx)=>AlertDialog(
        title: Text(_lang==AppLang.en ? 'Practice complete' : 'انتهى التدريب'),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(title: Text(_lang==AppLang.en ? 'Practice' : 'التدريب')),
      body: Padding(
